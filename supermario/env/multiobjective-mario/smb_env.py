...
class SuperMarioBrosEnv(NESEnv):
    ...
    def __init__(self,
        ...
        # a counter for suming up the reward with frames per step
        self._frame_counter = 0
        # elimination reward storage
        self._elimination_reward_storage = 0
        # setup a variable to keep track of the last frames time
        self._time_last = 0
        # setup a variable to keep track of the last frames x position
        self._x_position_last = 0
        # setup a variable to keep track of the last frames score
        self._score_last = 0
        # setup a variable to keep track of the last frames player status
        self._player_status_last = "small"
        # setup a variable to keep track of the last frames player status
        self._coins_last = 0
        # setup a variable to maintain the current multi-objective reward
        self.vec_reward = []
        # reset the emulator
        self.reset()
    ...
    @property
    def _elimination_reward(self):
        """Return the reward for enemy elimination"""
        if self._frame_counter == 0:
            self._elimination_reward_storage = 0
        self._frame_counter =  (self._frame_counter + 1) % self.frames_per_step
        _reward = (self._score - self._score_last) / 50 + self._elimination_reward_storage
        self._elimination_reward_storage = _reward
        self._score_last = self._score
        # TODO: check whether there are other cases for increasing score
        # If the score increase is due to player status change or collecting colins
        _player_status_last = self._player_status_last
        self._player_status_last = self._player_status
        if (self._player_status != _player_status_last) or \
           (self._coins != self._coins_last):
           return 0
    ...
    def _coin_reward(self):
        """Return the reward for enemy elimiation"""
    ...
    def _get_reward(self):
        ...
        _x_reward = self._x_reward 
        _elimination_reward = self._elimination_reward
        _time_penalty = self._time_penalty 
        _death_penalty = self._death_penalty 
        _coin_reward = self._coin_reward
        self.vec_reward = [_x_reward, _elimination_reward, _time_penalty, _death_penalty, _coin_reward]

        return _x_reward + _elimination_reward + _time_penalty + _death_penalty + _coin_reward
